...
def armonechoice(beliefs, myalpha, mybeta):
    """
    Function i/o:
    Receive a vector of beliefs, alpha (epsilon), and beta.

    Choose an arm based on epsilon-greedy with myalpha as epsilon. 
    If exploration: pull a random arm.
    If exploitation: pull the arm with the highest belief.

    If exploration and the randomly chosen arm is the highest belief arm, 
    repeat the choice using softmax with all beliefs except the max belief arms, and beta.

    Return the arm to pull (index).
    """
    
    # Epsilon-greedy exploration
    choice = np.argmax(beliefs)  # Default to exploitation (highest belief)
    explore = 0
    if np.random.random() < myalpha:  # Explore with probability myalpha (epsilon)
        explore = 1  
        
    # If exploration and the randomly chosen arm happens to be the max belief arm,
    # repeat the choice using softmax excluding the max belief arms.
    if explore:
        secondsteparms = np.repeat(1, armsboss)
        secondsteparms[choice] = 0
        playvectorstep2 = secondsteparms * softmax(beliefs, mybeta)
        playvectorstep2 += epsilon  # Avoid division by zero
        playvectorstep2 /= playvectorstep2.sum()  
