...
def armonechoice(beliefs, myalpha, mybeta):
    # function i/o:
    # receive a vector of beliefs, alpha, and beta
    #
    # first choose an arm to pull, based on softmax with all beleifs and alpha. If the choice is 
    # explitation (i.e., pull the arm with the highest beliefs), keep this choice. If the choice is exploration 
    # (i.e., pull an arm that is not the highest belief), repeat the choice based on softmax with all beleifs except
    # arms with max beliefs, and beta. 
    # 
    # return the arm to pull (i.e., 1 or 7)
    playvectorstep1 = softmax(beliefs, myalpha)
    choice = np.argmax(beliefs)
    explore = np.random.binomial(n=1, p=playvectorstep1[choice])
    if not explore:
        # select all the arms except the "exploitation" arm form last round
        secondsteparms = np.repeat(1,armsboss)
        secondsteparms[choice] = 0
        playvectorstep2 = secondsteparms * softmax(beliefs, mybeta)
        # let's make sure we don't divide by zero and kill hours of supercomputer time like we did last week
        playvectorstep2 += epsilon
        playvectorstep2 = playvectorstep2 / playvectorstep2.sum()
        choice = np.where(np.random.multinomial(1, playvectorstep2))[0][0]
    return(choice)
